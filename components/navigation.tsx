...
export default function Navigation() {
  ...
  // Removed isScrolled scroll listener for better performance
  ...
  return (
    <>
      ...
      <nav
        className="fixed top-0 left-0 right-0 z-50 bg-[#0a0a0a] shadow-[0_2px_20px_rgba(0,0,0,0.65)] border-b border-orange-500/20"
      >
        {/* Orange accent line */}
          ...
          <div className="flex justify-between items-center h-[70px]">
            ...
            >
              <div className="relative h-11 w-11 flex-shrink-0 overflow-hidden rounded-full
                              ...
                              ring-0 group-hover:ring-2 group-hover:ring-orange-500/40
              ...
              <div className="flex flex-col leading-none">
                <span className="text-xl font-extrabold tracking-tight text-orange-500
                                 group-hover:text-orange-400 transition-colors duration-150">
                  Creative Group
                </span>
            ...
            <div className="hidden md:flex items-center gap-1 lg:gap-2">
              {navLinks.map((link) => (
                <button
                  ...
                  className={`relative px-3 py-1.5 text-sm font-semibold tracking-wide rounded-md
                ...
                >
                  {link.name}
                  <span
                    className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] bg-orange-500 rounded-full
                                transition-all duration-200
            ...
            <div className="hidden md:flex items-center gap-4">
              <a
                href="tel:9770747074"
                className="flex items-center gap-1.5 text-sm text-gray-300 hover:text-orange-500
                           transition-colors duration-150 group"
              >
                <div className="p-1.5 rounded-full bg-orange-500/10 group-hover:bg-orange-500/20 transition-colors duration-150">
              ...
              <button
                ...
                className="relative px-5 py-2 bg-orange-500 text-black text-sm font-bold
                           rounded-md overflow-hidden group
                           hover:shadow-[0_0_18px_rgba(249,115,22,0.4)]
            ...
            <button
              ...
              className="md:hidden relative z-[60] flex h-10 w-10 items-center justify-center
                         ...
                         text-gray-300 hover:text-orange-500 hover:border-orange-500/50
      ...
      {/* Mobile Drawer */}
      <div
        className={`fixed top-0 right-0 z-50 h-full w-[275px] md:hidden
                    bg-[#0d0d0d] border-l border-orange-500/20 flex flex-col
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-800/80">
          ...
          <button
            onClick={() => setIsMobileMenuOpen(false)}
            className="p-1.5 rounded-full text-gray-400 hover:text-orange-500 hover:bg-gray-800 transition-colors duration-150"
          ...
          {navLinks.map((link, i) => (
            <button
              ...
              style={{
                ...
                transition: 'opacity 0.22s ease, transform 0.22s ease, background-color 0.15s, color 0.15s',
              }}
              className={`w-full flex items-center justify-between px-4 py-3.5 rounded-lg
                          text-left text-sm font-semibold border
                          ${activeLink === link.href
                            ? 'bg-orange-500/15 text-orange-500 border-orange-500/25'
                            : 'text-gray-300 hover:bg-gray-800/60 hover:text-orange-400 border-transparent'
                          }`}
            >
              <span>{link.name}</span>
              <ChevronRight className={`h-4 w-4 ${activeLink === link.href ? 'text-orange-500' : 'text-gray-600'}`} />
            </button>
          ))}
        </nav>
        ...
        <div className="px-5 py-5 border-t border-gray-800/80 space-y-3">
          <a
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-lg bg-gray-800/50
                       text-gray-300 hover:text-orange-500 transition-colors duration-150"
          >
            <div className="p-1.5 rounded-full bg-orange-500/15">
              <Phone className="h-4 w-4 text-orange-500" />
            </div>
            <span className="font-semibold tracking-wide text-sm">9770747074</span>
          </a>

          <button
            onClick={() => scrollToSection('#contact')}
            className="w-full py-3.5 bg-orange-500 hover:bg-orange-400 active:bg-orange-600
                       text-black font-extrabold text-sm rounded-lg
